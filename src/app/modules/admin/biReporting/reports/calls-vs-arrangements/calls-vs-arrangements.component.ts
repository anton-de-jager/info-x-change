...
@Component({
  selector: 'app-calls-vs-arrangements',
  imports: [
    CommonModule,
  ...
  templateUrl: './calls-vs-arrangements.component.html',
  styleUrl: './calls-vs-arrangements.component.scss'
})
export class CallsVsArrangementsComponent implements AfterViewInit, OnInit {
  ...
  tablePerformanceMetrics: any;
  columnsPerformanceMetrics: any;

  tableCallOutcomes: any;
  columnsCallOutcomes: any;
  ...
  constructor(
    ...
    this.selectPage('Calls vs Arrangements');
  }
  ...
  ngOnInit(): void {
    this.initTablePerformanceMetrics();
    this.initTableCallOutcomes();
  }
  ...
  initData(data) {
    this.data = data;
    this.filter = [];
    this.columnsPerformanceMetrics.forEach(column => {
  ...
  updateDonut() {
    const totals = this.dataFiltered.reduce(
      (acc, curr) => {
        acc.debtor += +curr.debtor;
        acc.employer += +curr.employer;
        acc.invalidNumber += +curr.invalidNumber;
        acc.noAnswer += +curr.noAnswer;
        acc.noSelection += +curr.noSelection;
        acc.someoneElse += +curr.someoneElse;
        acc.voicemail += +curr.voicemail;
        return acc;
      },
      {
        debtor: 0,
    ...
    this.labelsDonut = ['Employer', 'No Selection', 'No Answer', 'Someone Else', 'Debtor', 'Voicemail', 'Invalid Number'];
  }
